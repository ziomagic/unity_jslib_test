...
function _SK_Gyroscope_init(gameObjNameStr, onDynamicCall) {
 ...
 window._skJsLibGyro = function() {
  ...
  function DYNCALL(funcName, payload, data) {
   ...
   Module.dynCall_viiiiii(callbacks.onDynamicCall, funcNameBuffer, funcNameBufferSize, payloadBuffer, payloadBufferSize, buffer, data.length);
   _free(payloadBuffer);
...
if (!Object.getOwnPropertyDescriptor(Module, "callbacks")) Module["callbacks"] = function() {
 abort("'callbacks' was not exported. add it to EXPORTED_RUNTIME_METHODS (see the FAQ)");
};
